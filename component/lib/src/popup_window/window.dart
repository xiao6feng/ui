...
class PopupWindowButton<T> extends StatefulWidget {
  ...
  @override
  _WindowButtonState createState() {
    return _WindowButtonState();
  }

  static _WindowButtonState? of(BuildContext context) {
    final _WindowScope? scope = context.dependOnInheritedWidgetOfExactType<_WindowScope>();
    return scope?.state;
  }
}
...
class _WindowButtonState<T> extends State<PopupWindowButton> {
...
class _WindowRoute<T> extends PopupRoute<T> {
  _WindowRoute(
  ...
  Widget buildPage(BuildContext context, Animation<double> animation, Animation<double> secondaryAnimation) {
    return CustomSingleChildLayout(
      delegate: _WindowLayout(position),
...
class _WindowLayout extends SingleChildLayoutDelegate {
  _WindowLayout(this.position);
  ...
  @override
  bool shouldRelayout(_WindowLayout oldDelegate) {
    return position != oldDelegate.position;
  }
}

class _WindowScope extends InheritedWidget {
  const _WindowScope({Key? key, this.state, required Widget child}) : super(key: key, child: child);

  final _WindowButtonState? state;
